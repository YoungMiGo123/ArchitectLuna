...
public sealed class CleanArchitectureBuildTests
{
    ...
    {
        // Handler/endpoint generation comes from the adapter, not the persistence provider, so
        // these exist regardless of which persistence (including "none") is configured.
        ...
        // The entity class itself is generated by the persistence provider (GenerateEntityPersistence),
        // so its folder — and whether it exists at all — depends on which provider is configured:
        //   - "none" deliberately generates no domain type (NullPersistenceGenerator), matching the
        //     vertical-slice suite, which never asserts an entity file for this persistence either.
        //   - "marten" writes to Domain/Documents (a plain POCO document, not an EF/InMemory "entity" —
        //     see MartenPersistenceGenerator's doc comment).
        //   - everything else (efcore-postgres/efcore-sqlserver/in-memory) writes to Domain/Entities.
        var domainEntitySubfolder = persistence switch
        {
            "none" => null,
            "marten" => "Documents",
            _ => "Entities",
        ...
        if (domainEntitySubfolder is not null)
        {
